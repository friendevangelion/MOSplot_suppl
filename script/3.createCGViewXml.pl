    ...
    if ($flagOfCOG) {
        ...
        foreach (@dataCDS) {
            if ($_->[2] eq "-") {
                if (scalar(@{$_})>3) {
                    $mouseover = $_->[5]."; ".$_->[6]."; ".$_->[4]."; ".$_->[0]." to ".$_->[1];
                    print FOUT "\t", '<feature color="', $COGcolor{substr($_->[6],0,1)}, '" decoration="counterclockwise-arrow" label="', $_->[5], '" mouseover="', $mouseover, '">', '<featureRange start="', $_->[0], '" stop="', $_->[1], '" /></feature>', "\n";
                } else {
